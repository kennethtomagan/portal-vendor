        ...
        <div class="bg-white dark:bg-gray-800 rounded-lg shadow-lg overflow-hidden space-y-6 mt-8">
            <div class="py-2">
                ...
                <div class="px-4">
                    ...
                    <div  v-if="!isCentral" class="md:flex md:flex-row space-y-2 md:space-y-0 py-5" index="1">
                        <div class="w-full md:mt-2 px-6 md:px-8 md:w-1/5">
...
export default {
    ...
    methods: {
        ...
        updateVariant() {
            this.loading = true;
                Nova.request().put('/api/product/variants/' + this.variant.id, this.data)
                .then(() => {
                    this.loading = false;
                    this.$emit('reload');
                    this.$emit('close');
